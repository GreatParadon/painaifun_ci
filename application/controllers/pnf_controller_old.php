...
class pnf_controller extends CI_Controller {
	...
	{
		$room = array('rs2' =>$this->pnf_model->getRoomList());
	...
	public function accommodations($roomId){
		if (!$roomId)
		{
			$roomId = 3;
			$room = array( 'rs' =>  $this->pnf_model->getRoomInfo($roomId),
						   'rs2' =>$this->pnf_model->getRoomList(),
						   'im' => $this->pnf_model->getImage($roomId));
			$this->load->view('accommodations/accommodation', $room);
		}
		else
		{
			$room = array( 'rs' =>  $this->pnf_model->getRoomInfo($roomId),
